"""Tests for utility helpers in ``powerbox.tools``."""
...
def test_bins_upto_boxlen_warning() -> None:
    with pytest.warns(
        FutureWarning,
...
def test_bins_upto_boxlen(xmax, ndim) -> None:
...
@pytest.mark.parametrize("xmax", [1, 10, np.pi])
@pytest.mark.parametrize("ndim", [1, 2, 3])
def test_bins_upto_maxmag(xmax, ndim) -> None:
    x = np.linspace(-xmax, xmax, 21)
    mag = _magnitude_grid([x] * ndim)
...
def test_angular_avg_nd_3(interpolation_method) -> None:
    x = np.linspace(-3, 3, 400)
    X, Y = np.meshgrid(x, x)
    r2 = X**2 + Y**2
    P = r2**-1.0
...
def test_weights_shape() -> None:
    ...
    with pytest.raises(ValueError, match="coords and weights must have the same shape"):
        angular_average(P, freq, bins=10, weights=weights, bins_upto_boxlen=True)
...
def test_error_coords_and_mask() -> None:
...
def test_interp_method() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=(
            r"Unknown interpolation method: 'abc'\. "
            r"Use 'linear', 'nan-aware', a callable, or None\."
        ),
    ):
        angular_average_nd(
            field=P,
...
def test_error_w_kmag_coords() -> None:
    x = np.linspace(-3, 3, 40)
    P = np.ones((40, 40, 40))
    X, Y = np.meshgrid(x, x)
...
@pytest.mark.parametrize("n", range(1, 3))
def test_angular_avg_nd(n) -> None:
    x = np.linspace(-3, 3, 40)
...
def test_angular_avg_nd_complex_interp() -> None:
    x = np.linspace(-3, 3, 400)
    X, Y = np.meshgrid(x, x)
    r2 = X**2 + Y**2
    P = r2**-1.0 + 1j * r2**-1.0
...
def test_angular_avg_nd_4_2(interpolation_method) -> None:
...
def test_angular_avg_nd_2_1_varnull() -> None:
    x = np.linspace(-3, 3, 200)
...
def test_null_variance_2d() -> None:
    x = np.linspace(-3, 3, 400)
    X, Y = np.meshgrid(x, x)
    r2 = X**2 + Y**2
    P = np.ones_like(r2)
...
def test_angular_average_shape_exceptions() -> None:
...
def test_angular_averaged_nd_shape_exceptions() -> None:
...
class TestInterpSimilarToNoInterp:
    ...
    def test_angular_average_2d(self, interpolation_method) -> None:
        ...
        avg_none, _k_none, _, _sw_none = angular_average(
        ...
        with warnings.catch_warnings():
            ...
            avg_interp, _k_interp, _, _sw_interp = angular_average(
    ...
    def test_angular_average_3d(self, interpolation_method) -> None:
    ...
    def test_angular_average_nd_2d_field(self, interpolation_method) -> None:
        ...
        avg_none, _k_none, *_ = angular_average_nd(
        ...
        with warnings.catch_warnings():
            ...
            avg_interp, _k_interp, *_ = angular_average_nd(
                field=P,
                coords=freq[:2],
                bins=30,
                interpolation_method=interpolation_method,
